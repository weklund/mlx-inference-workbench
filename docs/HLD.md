...
2. Apple Silicon thermal behavior is reproducible with cooldown discipline and throttle detection **when the machine is dedicated to the benchmark session**. Discarding throttled / contended iterations is necessary for clean measurements. **Validated empirically** on M5 Max (Phase 0.5 spike [`docs/spikes/005_thermal_reproducibility.md`](spikes/005_thermal_reproducibility.md): protocol CoV **2.57%** on tok/s, N=25). Concurrent GPU/AI load can collapse throughput and must be excluded from official runs.
...
- **Risk: Thermal behavior is not reproducible enough.** **Status (2026-07-10): mitigated / hard gate passed for dedicated sessions.** Phase 0.5 spike [`docs/spikes/005_thermal_reproducibility.md`](spikes/005_thermal_reproducibility.md): protocol (AC + high performance + powermetrics + exclusive use) achieved **CoV 2.57%** on tok/s (N=25, 2 days). Within-session CoV typically &lt;2%. Residual: ~4% day-to-day mean drift — prefer same-session compares for small effects. **Not mitigated without discipline:** concurrent GPU/AI load produced CoV ~84% in one evening block (excluded from protocol). Mitigation in force: CoV threshold; exclusive-use policy for official runs; exclude battery / Heavy pressure / elevated pre-run GPU; cooldown. Remaining work: harness should contest on **GPU/combined** power (CPU-only gate missed GPU contention). Likelihood (protocol): Low. Likelihood (shared load, untreated): High. Impact if ignored: High.
...
- **Risk: `powermetrics` thermal monitoring is insufficient.** **Partial update (Phase 0.5):** passwordless `sudo powermetrics` on the lab host works for thermal pressure + CPU/GPU power and was required for protocol rows. It **does** surface Heavy pressure and multi-watt GPU draw; the gap is harness policy (CPU-only contestation gate), not complete sensor blindness. Fallback (timing anomaly) remains for degraded installs. Likelihood: Low–Medium. Impact: Medium (missed GPU contention without combined-power checks).
